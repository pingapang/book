...
<<dummy_1, fig.height=3.5, echo=FALSE, fig.align='center', message=F, results="asis", warning=F>>=
set.seed(123)
...
<<dummy_2, fig.height=3.5, echo=FALSE, fig.align='center', message=F, results="asis", warning=F>>=
window <- c(0, 0, 1, 1, 0)
data.bus <- data.frame(person, seat, window, price)
data.bus %>%
  head() %>%
  xtable(caption = "Bus trips to Paris.", 
         label = "tab:dummy_2", 
         digits = c(0, 0, 0, 0, 2)) %>%
  print(include.rownames = F, caption.placement = "top")
...
\begin{equation}
\widehat{\texttt{price}} = \Sexpr{out.bus$coef[1]} + \Sexpr{out.bus$coef[2]} \times \texttt{window}
\end{equation}

If the variable \texttt{window} has the value 1, then the expected or predicted price of the bus ticket is, according to this equation, $\Sexpr{out.bus$coef[1]} + \Sexpr{out.bus$coef[2]}\times  1= \Sexpr{out.bus$coef[1]+out.bus$coef[2]}$. What does this mean? Well, all persons who had a window seat also had a value of 1 for the \texttt{window} variable. Therefore the expected price of a window seat equals \Sexpr{out.bus$coef[1]+out.bus$coef[2]}. By the same token, the expected price of an aisle seat (\texttt{window = 0}) is $\Sexpr{out.bus$coef[1]} + \Sexpr{out.bus$coef[2]}\times 0= \Sexpr{out.bus$coef[1]}$, since all those with an aisle seat scored 0 on the \texttt{window} variable.
...
<<dummy_3,fig.height=3.5, message = F, echo=FALSE, fig.align='center', fig.cap='Relationship between dummy variable window and price.'>>=
data.bus %>% 
  ggplot(aes(x = window, y = price)) +
  geom_point() +
...
<<dummy_4,fig.height=3.5, echo=FALSE, fig.align='center', fig.cap='Relationship between type of seat and price.'>>=
data.bus %>% 
  ggplot(aes(x = seat, y = price)) +
  geom_point() +
...
<<dummy_5, fig.height=3.5, echo=FALSE, fig.align='center', message=F, results="asis", warning=F>>=
data.bus <- data.frame(person, 
                       seat, 
                       window, 
                       price, 
                       e = price - predict(out.bus), 
                       e_squared = (price - predict(out.bus))^2)
data.bus %>%
  head() %>%
  xtable(caption = "Bus trip to Paris data, together with residuals and squared residuals from the least squares regression line.", 
         label = "tab:dummy_5") %>%
  print(include.rownames = F, caption.placement = "top")
@
...
<<dummy_6,fig.height=3.5, echo=FALSE, fig.align='center', fig.cap='Relation between type of seat and price, with the regression line being not quite the least squares line.'>>=
data.bus %>% 
  ggplot(aes(x = seat, y = price)) +
  geom_point() +
...
<<dummy_7, fig.height=3.5, echo=FALSE, fig.align='center', message=F, results="asis", warning=F>>=
...
data.bus <- data.frame(person, 
                       ...
                       price, 
                       wrongpredict, 
                       e = price - wrongpredict, 
                       e_squared = (price - wrongpredict)^2)
data.bus %>%
  head() %>%
...
<<dummy_8,fig.height=3.5, echo=FALSE, fig.align='center', fig.cap='Data on yield under two experimental conditions: treatment 1 and treatment 2.'>>=
set.seed(12345)
data("PlantGrowth")
data <- PlantGrowth %>% 
  filter(group != "ctrl")
...
With treatment 1, the average yield turns out to be \Sexpr{mean(data$weight[data$group=="trt1"])}, and with treatment 2, the average yield is \Sexpr{mean(data$weight[data$group=="trt2"])}. Suppose we make a new dummy variable \texttt{treatment2} that is 0 for treatment 1, and 1 for treatment 2. Then we have the linear equation:
...
\begin{equation}
\label{weight}
\widehat{\texttt{weight}} = \Sexpr{mean(data$weight[data$group=="trt1"])} + \Sexpr{mean(data$weight[data$group=="trt2"])-mean(data$weight[data$group=="trt1"])}\times \texttt{treatment}
\end{equation}
...
<<dummy_9, fig.height=3.5, echo=FALSE, fig.align='center', message=F, results="asis", warning=F>>=
PlantGrowth %>% 
  filter(group != "ctrl") %>% 
  lm(weight ~ group, .) %>% 
  tidy(conf.int = TRUE) %>% 
...
The researchers analyse the data and they find the regression table as displayed in Table \ref{tab:dummy_9}. The 95\% confidence interval for the slope is from \Sexpr{round(confint(lm(weight ~ group, data))[2,1], 2)} to \Sexpr{round(confint(lm(weight ~ group, data))[2,2], 2)}. This means that reasonable values for the \textit{population} difference between the two treatments on yield lie within this interval. All these values are positive, so we reasonably believe that treatment 2 leads to a higher yield than treatment 1. We know that it is treatment 2 that leads to a higher yield, because the slope in the regression equation refers to a variable \texttt{grouptrt2} (see Table \ref{tab:dummy_9}). Thus, a dummy variable has been created, \texttt{grouptrt2}, where trt2 has been coded as 1 (and trt1 consequently coded as 0). In the next section, we will see how to do this yourself.

If the researchers had been interested in testing a null-hypothesis about the differences in mean yield between treatment 1 and 2, they could also use the 95\% confidence interval for the slope. As it does not contain 0, we can reject the null-hypothesis that there is no difference in group means at an $\alpha$ of 5\%. The exact $p$-value can be read from Table \ref{tab:dummy_9} and is equal to \Sexpr{round(summary(lm(weight ~ group, data))$coef[2,4], 2)}.

Thus, based on this regression analysis the researchers can write in a report:

\begin{quotation}
"There is a significant difference between the yield after treatment 1 and the yield after treatment 2, $t(18) = 3.01, p=\Sexpr{round(summary(lm(weight ~ group, data))$coef[2,4],2)}$. Treatment 2 leads to a yield of about \Sexpr{round(summary(lm(weight ~ group, data))$coef[2,1], 2)} (SE = \Sexpr{round(summary(lm(weight ~ group, data))$coef[2,2],2)}) more than treatment 1 (95\% CI: \Sexpr{round(confint(lm(weight ~ group, data))[2,1], 2)}, \Sexpr{round(confint(lm(weight ~ group, data))[2,2],2)})."
\end{quotation}
...
<<glimpse_growth, fig.height=3.5,  echo=TRUE, evaluate = T, fig.align='center', message=F, warning=F, fig.cap = "Looking at the type of variables in an R data frame.">>=
...
data.bus1 <- data.bus %>% 
  mutate(leg_room = c(60, 80, 80, 60, 70))

# # data.bus <- data.bus %>%           
# #   mutate(leg_room = c(80, 70, 70, 60, 70) 
# # )
# 
# 
...
# 
# data.bus1 %>%
#   ggplot(aes(x = leg_room, y = price, col = window)) +
#   geom_point(size = 3) +
#   geom_line(aes(y = pred), size = 2) +
#   # geom_abline(intercept = bus$coef[1], slope = bus$coef[3], col = "red", size = 2) +
#   # geom_abline(intercept = bus$coef[1] + bus$coef[2], slope = bus$coef[3], col = "lightblue", size = 2) +
#   # ylim(c(0, 71)) +
#   # xlim(c(0, 70)) +
#   xlab("Leg room in centimetres") +
...
data.bus1 %>% 
  lm(price ~ window + leg_room, data = .) %>% 
  tidy() %>% 
  xtable(caption = "Regression table for the regression of price on the dummy variable window and the numeric variable legroom.", 
...
\begin{equation}
\widehat{\texttt{price}} = \Sexpr{round(bus$coef[1], 2)} + \Sexpr{round(bus$coef[2],2)} \times \texttt{window} + \Sexpr{round(bus$coef[3], 2)} \times \texttt{legroom}  \label{eq:two_predictors}
\end{equation}



From Chapter \ref{chap:simple} we know how to interpret the coefficients. The slope coefficient for \texttt{window} should be interpreted as "the increase in price if we change seat from aisle to window, given a certain amount of legroom". That is, holding \texttt{legroom} constant, for instance at 60 centimetres, the difference between an aisle and a window seat equals \Sexpr{round(bus$coef[2],2)} Euros. And of course, this difference is also \Sexpr{round(bus$coef[2],2)} Euros when legroom equals 70 centimetres, and also when legroom equals 80 centimetres. Along the same vein, the slope coefficient for \texttt{legroom} should be interpreted as "the increase in price for every unit increase in \texttt{legroom}, given a certain type of seat". Thus, for an aisle seat, you pay \Sexpr{round(bus$coef[3],2)} Euros more for every extra centimetre. And this is also true for window seats: every extra centimetre for your legs costs you \Sexpr{round(bus$coef[3],2)} Euros.

The intercept of \Sexpr{round(bus$coef[1],2)} means that the model predicts that you pay \Sexpr{round(bus$coef[1],2)} if you happen to have 0 centimetres of leg room and an aisle seat (the reference category). Of course, a seat with 0 leg room does not exist, but it is simply what the model predicts, based on the data that are observed. The data and these predictions are visualised in Figure \ref{fig:leg_room_figure}.


<<leg_room_figure, echo = F, fig.height = 3.5, fig.cap="The bus trip to Paris data, with the predictions from a linear model with legroom and window as independent variables.">>=
library(modelr)
...
\begin{equation}
\widehat{\texttt{price}}  = \Sexpr{bus$coef[1]} + \Sexpr{bus$coef[2]}\times 0 + \Sexpr{bus$coef[3]} \times \texttt{legroom} = \Sexpr{bus$coef[1]} + \Sexpr{bus$coef[3]} \times \texttt{legroom} 
\end{equation}

Thus, the regression line for aisle seats has an intercept of \Sexpr{bus$coef[1]} and a slope of \Sexpr{bus$coef[3]}. Now let's fill in the equation for window seats, that is, \texttt{window = 1}. Then we obtain


\begin{equation}
\widehat{\texttt{price}}  = \Sexpr{bus$coef[1]} + \Sexpr{bus$coef[2]} \times 1 + \Sexpr{bus$coef[3]} \times \texttt{legroom} = \Sexpr{bus$coef[1] + bus$coef[2]} + \Sexpr{bus$coef[3]} \times \texttt{legroom} 
\end{equation}

That is, the regression line for window seats has an intercept that is different: it is equal to the original intercept plus the slope of the \texttt{window} variable, $\Sexpr{bus$coef[1]} + \Sexpr{bus$coef[2]} = \Sexpr{bus$coef[1] + bus$coef[2]}$. On the other hand, the slope for \texttt{legroom} is unchanged. With the same slope for \texttt{legroom}, the two regression lines are therefore parallel. 
...
$$5.03 - 0.37 \times 1 + 0.49 \times 0  =  5.03 - 0.37   = \Sexpr{5.03 - 0.37}$$
...
$$5.03 - 0.37 \times 0 + 0.49 \times 1 = 5.03 + 0.49 = \Sexpr{5.03 + 0.49}$$
...
<<example_anova, warning = F, message = F, fig.height = 4, echo = FALSE, fig.align = 'center', fig.cap = 'Illustration of ANOVA using a very small data set. In grey the raw data, in black the overall sample mean, and in red the sample group means.'>>=
set.seed(123)
group <- c("A", "B", "C") %>%
  as.factor() %>%
  rep(each = 3)
...
To illustrate the idea of ANOVA and the computation of the $F$-statistic, let's assume we have a very small data set, involving height data from three countries A, B and C. From each country, we only have 3 data points. The data are plotted in Figure \ref{fig:example_anova}. In grey, we see the raw data values for variable $Y$. In group A, we see the values 4, 2 and 1; in group B, we see the values 4, 2 and 2, and in group C, we see the values 2, 1 and 1. When we sum all these values and divide by 9, we get the overall mean (the grand mean), which is equal to $\widebar{Y} = \Sexpr{mean(y)}$, denoted in black in Figure \ref{fig:example_anova}. In red, we see the sample group means. For group A, that is equal to $(4 + 2 +1)/3=\Sexpr{(4 + 2 + 1)/3} $, for group B this is $(4 + 2 + 2)/3=\Sexpr{(4 + 2 + 2)/3} $, and for group C this equals $(2 + 1 +1)/3=\Sexpr{(2 + 1 +1)/3} $.
...
\begin{eqnarray}
\widehat{Y} = \Sexpr{(4 + 2 +1)/3} \texttt{dummy}_A + \Sexpr{(4 + 2 + 2)/3} \texttt{dummy}_B + \Sexpr{(2 + 1 +1)/3} \texttt{dummy}_C
\end{eqnarray}
...
<<anova_data_table,fig.height=3.5, echo=FALSE, message = F, fig.align='center', fig.cap='Small data example for illustrating ANOVA and the $F$-statistic.', results='asis'>>=
...
predicted <- tibble(y, group) %>%
  ...
  as.matrix() %>% as.vector() %>% 
  rep(each = 3)
data <- tibble(Y=y, group, dummy_A = c(1, 1, 1, rep(0, 6)),
           dummy_B = c(0, 0, 0, rep(1, 3), rep(0, 3)),
           dummy_C = c(rep(0, 6), rep(1, 3)),
           predicted, residual = y-predicted) 
data %>% 
  xtable(caption = "Small data example for illustrating ANOVA and the $F$-statistic.", 
...
data %>% 
  ...
  anova() %>% 
  tidy() %>% 
  xtable(caption = "ANOVA table for small data example.", 
...
<<anova_data2,fig.height=3.5, message = F, echo=FALSE, fig.align='center', fig.cap='Small data example for illustrating ANOVA and the $F$-statistic.', results='asis'>>=
...
\begin{equation}
F = \frac{MSB_{\texttt{group}}}{MSE} = \frac{\Sexpr{sum((predicted-2.111111)^2)/2}}  {\Sexpr{sum(data$residual^2)/6}}=
\Sexpr{anova(out)$F[1]}
\end{equation}
...
<<F2_6, warning = F, message = F, fig.height = 3.5, echo = FALSE, fig.align = 'center', fig.cap = 'Density plot of the $F$-distribution with 2 and 6 degrees of freedom. In blue the observed $F$-statistic in the small data example, in red the critical value for an $\\alpha$ of 0.05. The blackened area under the curve is 5$\\%$.'>>=
...
\begin{quote}
"The null-hypothesis that all 3 population means are equal was tested with an analysis of variance. The results showed that the null-hypothesis cannot be rejected, $F(2,\ 6) = \Sexpr{round(anova(out)$F[1], 2)}, MSE = \Sexpr{round(sum(data$residual^2)/6, 2)}, p = .40$."
\end{quote}
...
<<dummy_24,fig.height=3.5, echo=FALSE, fig.align='center', fig.cap='The vertical line represents a $t$-value of -2.40. The shaded area represents the extreme 5$\\%$ of the possible $t$-values'>>=
...
lb = -20; ub = qt(0.025, df = 40)
    ...
    (ggplot()
     ...
     + geom_area(data = area, 
                 mapping = aes(x = seq(qt(0.975, df = 40), 
                                       5, 
                                       length.out = 100),  
                               y = dt(seq(qt(0.975, df = 40), 
                                          5, 
                                          length.out = 100), 
                                      df = df, 
                                      ncp = ncp)), 
...
<<dummy_25,fig.height=3.5, echo=FALSE, fig.align='center', fig.cap='The $F$-distribution with 1 model degrees of freedom and 40 error degrees of freedom. The shaded area is the upper 5$\\%$ of the distribution. The vertical line represents the square of -2.40: 5.76'>>=
