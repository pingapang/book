...
Now if we take every possible $T$-value and square it, we can plot these and look at the distribution (see Figure \ref{fig:dummy_24}). It turns out that this is an $F$-distribution with 1 model degrees of freedom and 40 residuals degrees of freedom. If we square the $T$-value of -2.40, we obtain a value of 5.76. From online tables, we know that the proportion of $F$-values larger than 5.76 equals \Sexpr{1-pf(5.76,1,40)}. The proportion of $T$-values larger than 2.40 or smaller than -2.40 equals also \Sexpr{round(2*pt(-2.40,40),2)}. Thus, the two-sided $p$-value associated with a certain $T$-value, is equal to the $p$-value associated with an $F$-value that is the square of the $T$-value. 
