...
With dummy coding, we make a new variable that only has values 0 and 1, and that conveys the same information as the \textbf{seat} variable. The resulting variable is called a dummy variable. Let's call this dummy variable \textbf{aisle} and give it the value 1 for all persons that travelled in an aisle seat. We give the value 0 for all persons that travelled in a window seat. We can also call the new variable \textbf{aisle} a boolean variable with TRUE and FALSE, since in computer science, TRUE is coded by a 1 and FALSE by a 0. Whatever you want to call it, the data matrix including the new variable is displayed in Table \ref{tab:dummy_2}.

<<dummy_2, fig.height=4, echo=FALSE, fig.align='center', message=F, results="asis", warning=F>>=
aisle <- c(1,1,0,0,1)
data.bus <- data.frame(person, seat, aisle, price )
data.bus %>%
        head() %>%
        xtable(caption="Bus trips to Paris.", label="tab:dummy_2") %>%
        print(include.rownames=F, caption.placement = "top")
out.bus <- lm(price ~ aisle)
# source('/Users/stephanievandenberg/Dropbox/Statistiek_Onderwijs/Data Analysis/writeForeignCode.R')
# write.foreign(data.train,
#               '/Users/stephanievandenberg/Dropbox/Statistiek_Onderwijs/Data Analysis/spss examples logistic/train.sav',
#               '/Users/stephanievandenberg/Dropbox/Statistiek_Onderwijs/Data Analysis/spss examples logistic/train.sps',
...
What we have done now is coding the old qualitative variable \textbf{seat} into a quantitative variable \textbf{aisle} with values 0 and 1. Let's see what happens if we use a linear model for the variables price (dependent variable) and aisle (independent variable). The linear model is:

\begin{eqnarray}
price = b_0 + b_1 aisle + e \\
e \sim N(0,\sigma^2_e)
\end{eqnarray}
...
\begin{equation}
\hat{price} = \Sexpr{out.bus$coef[1]} + \Sexpr{out.bus$coef[2]} aisle
\end{equation}

If the variable \textbf{aisle} is 0, then the expected or predicted price of the bus ticket is, according to this equation, $\Sexpr{out.bus$coef[1]} + \Sexpr{out.bus$coef[2]}\times  0= \Sexpr{out.bus$coef[1]}$. What does this mean? Well, all tickets from persons with a window seat were coded as a 0. Therefore the expected price of a window seat equals \Sexpr{out.bus$coef[1]}. By the same token, the expected price of an aisle seat (aisle = 1) is $\Sexpr{out.bus$coef[1]} + \Sexpr{out.bus$coef[2]}\times 1= \Sexpr{out.bus$coef[1]+out.bus$coef[2]}$.
