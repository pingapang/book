...
<<summary_plot0, fig.height=5, echo=FALSE, fig.cap='Two regression lines: one for low SES children and one for high SES children.'>>=
...
<<summary_plot, fig.height=5, echo=FALSE, fig.cap="Two regression lines for the relationship between Age and Vocabulary Size, one for low SES children (SES=0) and one for high SES children (SES=1).">>=
...
<<summary_plot1, fig.height=4, echo=FALSE, fig.align='center', fig.cap="The effect of age on height.">>=
...
In Figure \ref{fig:interactionheightcountrysex} we see the relevant output. Table \ref{} what variables SPSS has created automatically.
...
data$sex <- data$sex %>% factor(labels=c("Female", "Male"))




data %>% filter(country != "C") %>% 
        ggplot(aes(x = country, y = height, fill=sex)) +
...
<<country_sex2, fig.height=4, echo=FALSE, fig.align='center', fig.cap="Average weights for males and females in countries A and B.">>=


data %>% filter(country != "C") %>% 
        ggplot(aes(x = sex, y = height, fill=country)) +
        geom_col(position='dodge' )
...
# means <- c( 175,155, 190  ,175)
# 
# data_frame(sex, country,means) %>% ggplot( aes(x=country, y=means, fill=sex))  +
...
data_frame(income, education, value) %>% 
...
write.foreign(data_frame(income, education, value) ,
...
<<linearbylinear_2, fig.height=4, echo=FALSE, fig.align='center', message=F, results="asis", warning=F, fig.cap="Sample data on gross yearly income, number of educational years and home market value.">>=
