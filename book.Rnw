...
The standard deviation of the sample mean is called the $\Sexpr{out.population$coef[2]}$. Had the population slope been 110 or -40, the sample slopes would cluster around 110 or -40, but the standard deviation of the sample slopes, the standard error, would be the same.
...
The black areas represent the area where $p < \alpha$: for all values of $T$ smaller than $\Sexpr{qt(0.01, df=df, ncp=0)}$ and larger than $\Sexpr{qt(0.99, df=df, ncp=0)}$, we reject the null-hypothesis. The probability that this happens, \textit{if the null-hypothesis is true}, is equal to $\alpha$ which is 0.01 in this example. The probability that this happens \textit{if the alternative hypothesis is true} (here, population slope is 1), is depicted in Figure \ref{fig:inf_21}.


<<inf_20,fig.height=4, echo=FALSE, fig.align='center', fig.cap='Different regression lines for different values of y if x=3.'>>=

df = 100; ncp = 0; limits = c(-5,5)
lb=-20; ub=qt(0.01, df=df)
    x <- seq(limits[1], limits[2], length.out = 100)
    xmin <- max(lb, limits[1])
    xmax <- min(ub, limits[2])
    areax <- seq(xmin, xmax, length.out = 100)
    ...
    (ggplot()
     + geom_line(data.frame(x = x, y = dt(x, df=df, ncp=ncp)),
                 mapping = aes(x = x, y = y))
            + geom_line(data.frame(x = x, y = dt(x, df=df, ncp=1)),
                 mapping = aes(x = x, y = y))
     + geom_area(data = area, mapping = aes(x = x,  y = ymax))
     + scale_x_continuous(limits = limits, breaks=seq(-5,5,1))
     + geom_area(data = area, mapping = aes(x = seq(qt(0.99, df=df) , 5, length.out = 100),  y = dt(seq(qt(0.99, df=df), 5, length.out = 100), df=df, ncp=ncp))) )
@

<<inf_21,fig.height=4, echo=FALSE, fig.align='center', fig.cap='Different regression lines for different values of y if x=3.'>>=
...
lb=-20; ub=qt(0.01, df=df)
    ...
    (ggplot()
     + geom_line(data.frame(x = x, y = dt(x, df=df, ncp=ncp)),
            ...
            + geom_line(data.frame(x = x, y = dt(x, df=df, ncp=1)),
                 mapping = aes(x = x, y = y))
     + geom_area(data = area, mapping = aes(x = x,  y = dt(x, df=df, ncp=1)))
     + scale_x_continuous(limits = limits, breaks=seq(-5,5,1))
     + geom_area(data = area, mapping = aes(x = seq(qt(0.99, df=df, ncp=0) , 5, length.out = 100),  y = dt(seq(qt(0.99, df=df, ncp=0), 5, length.out = 100), df=df, ncp=1)))
            +xlab("T"))
@

The area that is black in Figure \ref{fig:inf_21} turns out to be $\Sexpr{1-pt(2.364217, df=100, ncp=1) + pt(-2.364217,df=100,ncp=1)}$. This represents the probability that we find a signficant effect, \textit{if the population slope is 1}. This is actually the \textit{complement} of the probability to find a non-significant effect, \textit{if the population slope is 1}, which is $\beta$ or the probability of a type II error. Therefore, Figure \ref{fig:inf_21} represents $1- \beta$: the probability of finding a significant $p$-value, if indeed the null-hypothesis is not true. In this example, $1-\beta$ is equal to $\Sexpr{1-pt(2.364217, df=100, ncp=1) + pt(-2.364217,df=100,ncp=1)}$, so $\beta$ is equal to its complement, $1- \Sexpr{1-pt(2.364217, df=100, ncp=1) + pt(-2.364217,df=100,ncp=1)} = \Sexpr{1- (1-pt(2.364217, df=100, ncp=1) + pt(-2.364217,df=100,ncp=1))}$.
...
<<inf_22,fig.height=4, echo=FALSE, fig.align='center', fig.cap='Different regression lines for different values of y if x=3.'>>=
...
<<inf_23,fig.height=4, echo=FALSE, fig.align='center', fig.cap='Different regression lines for different values of y if x=3.'>>=
