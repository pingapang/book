...
\begin{eqnarray}
Population: Volume &=& \Sexpr{round(out.population$coef[1],2)} + \Sexpr{round(out.population$coef[2],3)} \times temperature  \nonumber\\
Sample: Volume &=&  \Sexpr{round(out.sample$coef[1],2)}  \Sexpr{round(out.sample$coef[2],4)} \times temperature \nonumber
\end{eqnarray}
...
Thus, in our example of the 200 bottles with a sample slope of $\Sexpr{round(out.sample$coef[2],3)}$ and a standard error of $\Sexpr{round(se,3)}$, the interval from $\Sexpr{round(out.sample$coef[2],3)}- 2.58* \Sexpr{se}$ to $\Sexpr{round(out.sample$coef[2],3)}+ 2.58* \Sexpr{round(se,3)}$ contains reasonable values for the population mean. If we do the calculation, we get the interval from $\Sexpr{round(out.sample$coef[2]- 2.58* se,2)}$ to $\Sexpr{round(out.sample$coef[2]+ 2.58* se,2)}$. If we would have to guess the value for the population slope, our guess would be that it would lie somewhere between between -0.30 and $\Sexpr{round(out.sample$coef[2]+ 2.58* se,2)}$, \textit{if we feel that 1\% is a small enough probability}.
