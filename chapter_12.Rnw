...
Thus, if you have a logoddsratio $x$ of $\Sexpr{round(predict.glm(outglm, newdata=data.frame(age=70)),2)}$, the odds ratio equals $exp(-0.34)=\Sexpr{round(exp(-0.34),2)}$, and
