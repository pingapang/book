...
The relig\_income dataset stores counts based on a survey which (among other things) asked people about their religion and annual income:

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# vignette("pivot")}
\hlstd{relig_income}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'relig\_income' not found}}\end{kframe}
\end{knitrout}

This dataset contains three variables:
\begin{enumerate}
\item religion, stored in the rows,
\item income spread across the column names, and
\item count stored in the cell values.
\end{enumerate}

To put the values that we see in the columns into one single column, we use pivot\_longer():
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in relig\_income \%>\% pivot\_longer(cols = -religion, names\_to = "{}income"{}, : could not find function "{}\%>\%"{}}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in us\_rent\_income \%>\% pivot\_wider(names\_from = variable, values\_from = c(estimate, : could not find function "{}\%>\%"{}}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in tibble(age) \%>\% ggplot(aes(age)): could not find function "{}\%>\%"{}}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in tibble(value) \%>\% count(value, name = "{}frequency"{}) \%>\% mutate(proportion = frequency/8) \%>\% : could not find function "{}\%>\%"{}}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in tibble(age) \%>\% count(age, name = "{}frequency"{}) \%>\% mutate(proportion = frequency/1000) \%>\% : could not find function "{}\%>\%"{}}}\end{kframe}
