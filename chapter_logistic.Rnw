...
<<gen_1, fig.height=3.5, echo=FALSE, fig.align='center', fig.cap='Density function of the normal distribution, with mean 0 and variance 4 (standard deviation 2). Inflexion points are positioned at residual values of minus 1 standard deviation and plus 1 standard deviation.'>>=
...
<<gen_2, fig.height=3.5, warning = F, echo=FALSE, fig.align='center', message=F, fig.cap='Even if residuals are really discrete, the normal distribution can be a good approximation of their distribution.'>>=
set.seed(1234)
residual <- rnorm(1000, 0, 3) %>% round(1)
density <- dnorm(residual, 0, 3)
data <- tibble(residual, density)
data %>% 
  ggplot(aes(x = residual, y = density)) +
...
<<gen_3, fig.height=3.5, echo=FALSE, warning = F, fig.align='center', message=F, fig.cap='Count data example where the normal distribution is not a good approximation of the distribution of the residuals.'>>=
set.seed(1234)
...
<<gen_4, fig.height=3.5, warning = F,  echo=FALSE, fig.align='center', message=F, fig.cap='Dichotomous data example where the normal distribution is not a good approximation of the distribution of the residuals.'>>=
set.seed(1234)
score <- rnorm(100, 1, 3) %>% round(0)
score[score <= 4 ] <- 0
score[score>3] <- 1
...
<<gen_5, fig.height=3.5, echo=FALSE, fig.align='center', message=F, fig.cap='Data example: Exam outcome (score) as a function of age, where 1 means pass and 0 means fail.'>>=
set.seed(12345)
age <- rnorm(100, 78, 3) %>% round(1)
logoddsscore <- 0.51*(age-78) %>% round(0)
score <- exp(logoddsscore)/ (1 + exp(logoddsscore))
# score[score <= 4] <- 0
# score[score>3] <- 1
score  <- rbernoulli(100, score) %>% as.numeric()
data.exam <- tibble(score, age)
data.exam %>% 
...
<<gen_6, fig.height=3.5, echo=FALSE, fig.align='center', message=F, , fig.cap='Example exam data with a linear regression line.'>>=
data.exam %>% 
  ggplot(aes(x = age, y = score)) + 
  geom_point() + 
  xlab("age in months") + 
...
% 
% 
<<gen_7, fig.height=3.5, echo=FALSE, warning = F, fig.align='center', message=F, fig.cap='Residuals as a function of age, after a linear regression analysis of the exam data.'>>=
residual <- lm(score ~ age, data = data.exam)$res
data.exam <- tibble(residual, age, score)
data.exam %>% 
  ggplot(aes(x = age, y = residual)) + 
  geom_point() + 
  xlab("age in months")
outglm <- glm(score ~ age, family = binomial, data = data.exam)
@
...
\begin{eqnarray}
\texttt{logodds}=\Sexpr{round(outglm$coef[1],2)} + \Sexpr{round(outglm$coef[2],2)} \times \texttt{age}, \nonumber
\end{eqnarray}


This equation predicts that a child aged 70 months has a logodds of $\Sexpr{round(outglm$coef[1],2)} + \Sexpr{round(outglm$coef[2],2)} \times 70 =\Sexpr{round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)}$. In order to transform that logodds back to a probability, we first have to take the exponential of the logodds\footnote{If we know $\textrm{ln}(x)=60$, we have to infer that $x$ equals $e^{60}$, because $\textrm{ln}(e^{60})=60$ by definition of the natural logarithm, see previous footnote. Therefore, if we know that $\textrm{ln}(x)=c$, we know that $x$ equals $e^c$. The exponent of $c$, $e^c$, is often written as $\textrm{exp}(c)$. So if we know that the logarithm of the odds equals $c$, $\texttt{logodds}=\textrm{ln}(\texttt{oddsratio})=c$, then the odds is equal to $\textrm{exp}(c)$.} to get the odds:


\begin{eqnarray}
\texttt{odds} = \textrm{exp}(\texttt{logodds})= e^{\texttt{logodds}}=e^{\Sexpr{round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)}}=\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)} \nonumber
\end{eqnarray}

An odds of \Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)} means that the odds of passing the exam is \Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)} to 1 (\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}:1). So out of $1 + \Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}= \Sexpr{1+round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}$ times, we expect \Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)} successes and 1 failure. The probability of success is therefore $\frac{\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}}{1+\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}} = \Sexpr{round(round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)/ (1+(round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2))),2)}$. Thus, based on this equation, the expected probability of passing the exam for a child of 70 months equals $\Sexpr{round(round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)/ (1+(round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2))),2)}$.
...
Thus, if you have a logodds $x$ of $\Sexpr{round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)}$, the odds equals $\textrm{exp}(\Sexpr{round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)})=\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}$, and
the corresponding probability is $\frac{\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}}{1+\Sexpr{round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)}} = \Sexpr{round(round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2)/ (1+(round(exp(round(round(outglm$coef[1],2) + 70*round(outglm$coef[2],2) ,2)),2))),2)}$.
...
<<gen_9, fig.height=3.5, echo=FALSE, fig.align='center', message=F,fig.cap='Example of a linear model for the logit of probabilities of passing an exam.'>>=
age <- seq(70, 88, 1)
logoddsratio <- predict.glm(outglm, newdata = data.frame(age))
data <- data.frame(age, logoddsratio)
data %>% 
  ggplot(aes(x = age, y = logoddsratio)) + 
  geom_line() + ylab("logit(p) = logarithm of the odds") + 
  xlab("age in months") +
  geom_label(aes(x = 80, y = -1), 
             col='black', 
             label='intercept: -33.15, slope: 0.42', 
             show.legend = F)
@
% 
...
<<gen_10, fig.height=3.5, echo=FALSE, fig.align='center', message=F,fig.cap='Example with logodds transformed into probabilties (vertical axis).'>>=
probs <- exp(logoddsratio)/(1 + exp(logoddsratio))
data <- tibble(age, probs)
data %>% 
...
<<gen_11, fig.height=3.5, echo=FALSE, fig.align='center', message=F, fig.cap='Transformed regression line and raw data points.'>>=
data %>% 
  ggplot(aes(x = age, y = probs)) + 
  geom_line() + 
  ylab("probability of passing the exam") + 
  xlab("age in months")  +
  geom_point(data = data.exam, aes(x = age, y = score)) + 
  xlim(c(70, 88))
@
...
% 
% 
% We can interpret these results by making some predictions. Imagine a traveller with a yearly income of 11,000 Euros. Then the predicted logodds equals $90.017 - 0.008 \times 11000= \Sexpr{90.017 - 0.008 * 11000}$. When we transform this back to a probability, we get $\frac{exp(\Sexpr{90.017 - 0.008 * 11000}) } {1+ exp(\Sexpr{90.017 - 0.008 * 11000}) } = 0.88 $. So this model predicts that for people with a yearly income of 11,000, about 88\% of them take the train (if they travel at all, that is!).
% 
% Now imagine a traveller with a yearly income of 100,000. Then the predicted logodds equals $90.017 - 0.008 \times 100000 = -709.983 $. When we transform this back to a probability, we get $\frac{exp(\Sexpr{90.017 - 0.008 * 100000}) } {1+ exp(\Sexpr{90.017 - 0.008 * 100000}) }= 0.00$. So this model predicts that for people with a yearly income of 100,000, close to none of them take the train.
...
We can interpret these results by making some predictions. Imagine a traveller with a yearly income of 11,000 Euros. Then the predicted logodds equals $90.0 - 0.00817 \times 11000= \Sexpr{90.0 - 0.00817 * 11000}$. When we transform this back to a probability, we get $\frac{\textrm{exp}(\Sexpr{90.0 - 0.00817 * 11000}) } {1+ \textrm{exp}(\Sexpr{90.0 - 0.00817 * 11000}) }= 0.53  $. So this model predicts that for people with a yearly income of 11,000, about 53\% of them take the train (if they travel at all, that is!).

Now imagine a traveller with a yearly income of 100,000. Then the predicted logodds equals $90.0 - 0.00817 \times 100000 = -727 $. When we transform this back to a probability, we get $\frac{\textrm{exp}(\Sexpr{90.0 - 0.00817 * 100000}) } {1+ \textrm{exp}(\Sexpr{90.0 - 0.00817 * 100000}) }= 0.00$. So this model predicts that for people with a yearly income of 100,000, close to none of them take the train.
