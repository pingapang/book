...
<<nonpar_1, fig.height=4, echo=FALSE, fig.align='center'>>=
set.seed(1234567)
...
data.frame(AgeMother, Aggression) %>% 
        ggplot(aes(AgeMother, Aggression)) + geom_point() + ylab("Aggression") +geom_smooth(method="lm", se=F)
@
...
data_frame(AgeMother, Aggression2) %>% 
        ggplot(aes(AgeMother, Aggression2)) +geom_point() + geom_smooth(method="lm", se=F)
@
...
<<fig1201, fig.height=4, echo=FALSE, fig.align='center', warning=FALSE >>=
set.seed(1234567)
Aggression <- rmultinom(15, 3, prob=c(0.3, .3, .3)) +1
Aggression <- Aggression[1,]
AgeMother <- 29 + Aggression + rnorm(15, 0,1)
...
Suppose we have 10 students and we ask their teachers to rate them. One teachers rates them on geography and the other teacher rates them on history. We only ask them to give rankings: indicate the brightest student with a 1 and the dullest student with a 10. Then we might have the following data set:
